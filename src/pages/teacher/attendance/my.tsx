...
const attendanceRecords: TeacherAttendance[] = [
  {
    id: '1',
    date: '2025-05-01',
    status: 'present',
...
const TeacherMyAttendance: React.FC = () => {
  const [month, setMonth] = useState<string>(new Date().toISOString().split('-').slice(0, 2).join('-'));
  ...
  const stats = {
    ...
    attendancePercentage: Math.round((attendanceRecords.filter(r => r.status === 'present' || r.status === 'late').length / attendanceRecords.length) * 100)
  };
